...
In \figref{fig:pk_v2_ident}, the model shows good results in case if the pumps shut down and turn on for different WT levels and inlet pressures. Similarly to Validation 1, the biggest error between the model and measurement are at times when both pumps are completely shut down. 
...
The physical properties of the vertices are shown in \tabref{pipes_table_example1_nodes}.
...
\begin{center}
    ...
    \hline
    \end{tabular}
    \captionof{table}{Physical properties of vertices in the test network.}
