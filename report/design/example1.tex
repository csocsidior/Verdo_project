...
The output equation described in \eqref{identification_model} is going to be approximated for two pumping stations, with RBFs in the form as shown in \eqref{output_RBFNNnetwork_approx}.
...
The NN model of the output equation is shown in \figref{fig:nn_output}, with the corresponding parameter and weigh vectors.
 ...
 Furthermore, the output layer consists of the output neuron which computes $\bar{p}_{\mathcal{K},k}$ by summing the weighted RBFs, the weighted linear signals, the output biases and the constant inlet elevation parameters. Thus, identification for the inlet pressure vector $\bar{p}_{\mathcal{K},k} $ is carried out according to \eqref{inletpressures_ident_matrix}

  \begin{equation}
\label{inletpressures_ident_matrix}
\bar{p}_{\mathcal{K},k} = \theta^T_{\mathcal{K}} \chi(u_k),
\end{equation}

where the parameter matrix $\theta_{\mathcal{K}}$ and regression matrix $\chi$ is given such that

  \begin{equation}
\label{par_regr_matrix}
\theta_{\mathcal{K}} = 
          \begin{pmatrix}
           w_{\mathcal{K}1,1} & w_{\mathcal{K}2,1} \\[1pt]
           \vdots & \vdots \\[1pt]
           w_{\mathcal{K}1,M} & w_{\mathcal{K}2,M} \\[1pt]
           \tilde{a}_{\mathcal{K}1,1} & \tilde{a}_{\mathcal{K}2,1}\\[1pt]
           \vdots & \vdots \\[1pt]
           \tilde{a}_{\mathcal{K}1,l} & \tilde{a}_{\mathcal{K}2,l}\\[1pt]
           1 & 1\\[1pt]
           b_{\mathcal{K}1} & b_{\mathcal{K}2}\\[1pt]
         \end{pmatrix}
         ,
         \hspace{5mm}
         \chi = 
         \begin{pmatrix}
           ...
           \bar{h}_{\mathcal{K}} \\[1pt]
           1 \\[1pt]
         \end{pmatrix}.
\end{equation}

With a given input set $u_k$ and output $\bar{p}_{\mathcal{K},k}$ set, the optimal solution for the parameters is calculated by using LS method, as \eqref{inletpressures_ident_matrix} is a linear matrix equation. Thus, the parameter matrix is given by \eqref{inletpressures_ident_matrix1}

  \begin{equation}
\label{inletpressures_ident_matrix1}
 \theta^T_{\mathcal{K}} = \bar{p}_{\mathcal{K},k} \chi^{\dagger}(u_k), 
\end{equation}
...
The state equation, described in \eqref{identification_model} is going to be approximated with RBFs in the same way as it has been done for the inlet pressures. Therefore, the state derivative $\hat{p}_{k+1} \!-\! \hat{p}_k$ of one WT, Tank A is given in the form as shown in \eqref{state_der_ident_approx}


  \begin{equation}
\label{state_der_ident_approx}
 \hat{p}_{A,k+1} - \hat{p}_{A,k} = \sum_{i = 1}^N w_{A1,i} \phi_i(u_k) + \sum_{j = 1}^c \tilde{a}_{A1,j} \bar{d}_{\mathcal{K}j,k} + \tilde{a}_{A2} \sigma_k
\end{equation}
