...
where the Moore-Penrose pseudoinverse of the regression matrix $X_{\mathcal{K}1}$ is computed. $X_{\mathcal{K}1}$consists of as many $\chi_{\mathcal{K}1}$ regression vectors, as the number of samples. Furthermore, the number of samples $m$ need to fulfil \eqref{rankeq}.

\begin{equation}
\label{rankeq}
 m \geq rank(\chi_{\mathcal{K}1}(u^k)),
\end{equation}

\vspace{-3mm}

where $rank(\chi_{\mathcal{K}1}(u^k)) = r$. 
