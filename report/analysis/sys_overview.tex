...
The Randers drinking WSS is managed by Verdo A/S and is the main supplier of drinking water and heating to the city of Randers. Verdo supplies water to approximately 46.000 customers in Randers Municipality \cite{verdo}. The WSS in Randers is a complex, looped configuration with many different distribution areas. The coverage of the distribution areas are shown in \figref{fig:level_zones} bellow.

%illustration of tanks in the EPANET model of Randers WSS
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{report/pictures/level_zones1}
\caption{Distribution zones in the Randers WSS.}
\label{fig:level_zones}
\end{figure}

In \figref{fig:level_zones}, the different distribution areas are shown, however the whole distribution map can be split up into four main areas, according to the geographical properties of the city. The main four regions can be separated as

\begin{itemize}
   \item \emph{Area 1}: Includes the area labelled with HZ, (High Zone)
   \item \emph{Area 2}: Includes the area labelled with LZ, (Low Zone) 
   \item \emph{Area 3}: Includes the area labelled with HNP and HNP\_TR1 
   \item \emph{Area 4}: Includes the area labelled with VSV\_TF, VSV and VSV\_TR
 \end{itemize}
...
%   \item Are PRVs and FCVs really placed at the pumping stations in reality, or just for 
% \end{itemize}
...
% As can be seen, reservoirs are placed in the outer parts of the city, typically next to the main pumping stations. The location of the six tank stations is shown in (figref below)
