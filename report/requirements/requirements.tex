...
In elevated reservoirs, the rate of change in the volume of the fluid inside the tank is proportional to the volumetric flow at which water enters or leaves the tank. Since the pressure on the bottom is due to the amount of water, proportionality can be set between the pressure and the flow in and out of the tank. The dynamics of such a system can be described by a first order differential equation of the form
...
It is assumed that the tanks in the model and also in the WSS in Randers have the same cross sectional areas in the entire height of the tank. 
...
Now, defining a matrix $\Xi$, in which the arrangement of the edges are the same as for the incidence matrix, $H$, meaning that $\Xi$ is regarding to the graphs $H_{\mathcal{T}}$ and $H_{\mathcal{C}}$. $\Xi$ is defined as follows
...
It should be noted that the expressions in matrix $\Xi$ are of the same structure as the structure of a partitioned cycle matrix. However, the set $\mathcal{T}$ does not define a spanning tree when $c>0$, therefore matrix $\Xi$ is not a cycle matrix corresponding to any spanning tree. Multiplying $H$ with $\Xi$ from the left-hand side
...
$\Xi$ is defined such that $\Xi \bar{H}^T = 0$ by definition \cite{deo2017graph}. 

Multiplying with $\Xi$ from the left in \eqref{ohmslaw_matrixform} 
