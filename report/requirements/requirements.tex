...
$A$ has a non-trivial kernel, and for every unique value of $\frac{1}{\sigma^2} A (\hat{p}+\hat{h}) $, there is a unique solution for $a_{\mathcal{C}}$.
...
it is seen that the tank flows, $\hat{d}_t$, need to be expressed in terms of the inputs, $\hat{d}_c$. In order to express the demand regarding the tank, the whole network is treated as one node, as illustrated in \figref{fig:onenode_system}. 
...
According to the mass-balance in the network, and using the corresponding partitioning, as shown in \figref{fig:onenode_system} the balance equation on demand flows is in the form of
...
It is important to point out that this first order differential equation in \eqref{wt_model_demand}, describes a simple mass-balance model of a single tank system, where the input flows equal to the overall demand in the network and the rate of change in storage in the tank. 
